...
static long32 f(ulong32 r, unsigned char subkey[8])
{
	...
	*/
	unsigned char s[4], plaintext[8], m[4], t[4], temp;
	...
	for (i = 0; i != 8; ++i)
	{
		for (j = 0; j != 6; ++j)
		{
			...
			fromSource = plaintext_32bit_expanded_to_48bit_table[count - 1];
			temp = (s[(fromSource - 1) / 8] & bytebit[7 - (fromSource - 1) % 8]);
			//             32 -> find s[3]      
			if ((fromSource - 1) % 8 - j >= 0)
	...
	for (i = 0; i != 32; ++i)
	{
		...
		temp = (m[(fromSource - 1) / 8] & bytebit[(fromSource - 1) % 8]);
		if (((fromSource - 1) % 8 - (i % 8)) >= 0)
			t[i / 8] |= temp << ((fromSource - 1) % 8 - (i % 8));
		else
			t[i / 8] |= temp >> -((fromSource - 1) % 8 - (i % 8));
	}
	rval = 0;
